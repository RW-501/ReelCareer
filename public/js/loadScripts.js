  ...
  if (Array.isArray(location)) {
    ...
    switch (part) {
      ...
      case "state":
        ...
        location = `<a  class= 'loc-link' href="https://reelcareer.co/state#${encodeURIComponent(location[1] || "").toLowerCase().trim()}">${location[1] || ""}</a>`; // Return state link
        break;
      case "county":
        location = location[2] || ""; // Return the third part (county)
        break;
      case "city":
        location = `<a  class= 'loc-link' href="https://reelcareer.co/city#${encodeURIComponent(location[location.length - 1]).toLowerCase().trim()}">${location[location.length - 1]}</a>`; // Return city link
        break;
      default:
